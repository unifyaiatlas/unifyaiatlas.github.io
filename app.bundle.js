...
(function () {
  ...
  // ==================== src/pages/docs/docsPortal.js ====================
  // Architecture & Documentation Portal Page Component
  async function renderDocsPortalPage(params) {
    const urlParams = new URLSearchParams(window.location.hash.split('?')[1] || '');
    const activeTab = urlParams.get('tab') || (params && params.tab) || 'overview';
    ...
    return `
      ...
      <div class="card" style="padding: 24px;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
          ...
          <span class="badge badge-info">End-to-End Topology</span>
        </div>
  ...
  function renderMicroservicesTab() {
    const services = [
      { name: 'source-federation-service', port: 4001, icon: '🌐', desc: 'Controls Zero-Copy Lakehouse Federation adapters, external catalog views, and Delta Sharing protocol endpoints.', routes: ['POST /v1/federation/catalogs', 'GET /v1/federation/sources/:id/schema', 'POST /v1/federation/test-pushdown'], contract: 'SourceConnectionConfig, VirtualCatalogSpec, PushdownCapability' },
      { name: 'lakebase-metastore-service', port: 4002, icon: '🗄️', desc: 'Active system metastore residing directly inside Databricks Unity Catalog Delta tables (system.unify_lakebase).', routes: ['GET /v1/metastore/entities', 'POST /v1/metastore/entities/:id/version', 'PUT /v1/metastore/survivorship-matrix'], contract: 'CanonicalEntityModel, AttributeMappingRule, LakebaseMetadataSnapshot' },
      { name: 'layer-spawner-service', port: 4003, icon: '🏗️', desc: 'Provisions, drops, and clones physical or virtual database layers on Databricks Serverless compute on demand.', routes: ['POST /v1/layers/spawn', 'GET /v1/layers/active', 'POST /v1/layers/:id/clone-sandbox'], contract: 'SpawnLayerRequest, LayerProvisionResult, LayerLifecycleState' },
      { name: 'pipeline-orchestrator-service', port: 4004, icon: '🔄', desc: 'Compiles visual pipeline graphs into Databricks Asset Bundles (databricks.yml) and Delta Live Tables (DLT) python/sql specs.', routes: ['POST /v1/pipelines/compile-dabs', 'POST /v1/pipelines/deploy', 'GET /v1/pipelines/:id/runs'], contract: 'PipelineDagConfig, DltPipelineSpec, JobRunTelemetry' },
      { name: 'dq-intelligence-service', port: 4005, icon: '🛡️', desc: 'Runs schema profiling, Great Expectations assertions, and anomaly detection over live federated and Delta data.', routes: ['POST /v1/dq/profile-stream', 'GET /v1/dq/assertions/:entity', 'POST /v1/dq/quarantine-records'], contract: 'DqRuleSpec, ProfilingMetric, DqIssueAlert' },
      { name: 'unification-engine-service', port: 4006, icon: '⚡', desc: 'Executes Fellegi-Sunter probabilistic matching, fuzzy phonetic comparisons, and survivorship resolution algorithms.', routes: ['POST /v1/unification/match-batch', 'POST /v1/unification/simulate-weights', 'GET /v1/unification/clusters/:id'], contract: 'MatchStrategy, SurvivorshipMatrix, GoldenCluster' },
      { name: 'genie-zerocopy-query-service', port: 4007, icon: '🔮', desc: 'Configures Databricks Genie Spaces, injects semantic business context, and executes natural language zero-copy queries.', routes: ['POST /v1/genie/spaces/provision', 'POST /v1/genie/query-prompt', 'GET /v1/genie/explain-pushdown'], contract: 'GenieSpaceConfig, ZeroCopyQueryPlan, GeniePromptResponse' },
      { name: 'entity360-activation-service', port: 4008, icon: '🚀', desc: 'Exposes sub-millisecond GraphQL and REST Entity 360 endpoints, identity graph traversals, and reverse ETL writebacks.', routes: ['GET /v1/entities/:id/profile', 'GET /v1/entities/:id/graph', 'POST /v1/activation/sync-salesforce'], contract: 'Entity360Profile, IdentityGraphNode, ActivationJob' }
    ];
